...
/**
 * Clase que implementa el algoritmo RTA*
 * @author jrojas14
 */
public class AgenteRTAStarP4 extends Agente {
    static HashMap<Coordinates, Double> tabla_hash = new HashMap<Coordinates, Double>();   // tabla para actualizar valor heuristico
     ...
     * @param elapsedTimer 
     */
    public AgenteRTAStarP4(StateObservation stateObs, ElapsedCpuTimer elapsedTimer) throws IOException {
    ...
    /**
     * Calculo de la siguiente accion, segun actual y mejor_vecino
    ...
    public Types.ACTIONS busqueda() {
        ...
        // nos quedamos con el segundo mejor vecino (en getMejorVecinoRTAStar obtenemos el mejor vecino de actual y lo eliminamos de vecinos)
        Node peek = vecinos.peek();
        
        // si existe el segundo mejor vecino, actualizamos la heuristica
        if (peek != null) {
            tabla_hash.put(actual.coordenadas, Math.max(actual.geth(), peek.geth()+1));
        // si no, actualizamos la heuristica al mejor vecino
        } else {
    ...
    /**
     * Metodo que ejecuta la accion, segun RTA*
     * @param stateObs
     * @param elapsedTimer
     * @return accion a ejecutar
     */
